...
def main_loop():
    ...
    while running:
        ...
        # Build small moving window of torque or rpm to detect changes
        ...
        if state == "ASSIST":
            # Primary role: apply positive torque to spin ICE
            set_torque(axis, ASSIST_TORQUE_NM)

            # Detect reduction in resistance: if rpm climbs above baseline by threshold
            ...
            # Update baseline slowly (so that baseline adapts between cranking attempts)
            # but do not adapt too fast while cranking. Use exponential smoothing:
            rpm_baseline = rpm_baseline * 0.995 + rpm * 0.005

        elif state == "GENERATE":
            # Apply negative torque to absorb energy (generator/regenerative braking)
            # We should clamp the requested torque so we don't exceed current or brake resistor capacity.
            # Basic control: set a negative torque proportional to generated power target or fixed.
            set_torque(axis, GEN_MAX_NEG_TORQUE_NM)

            # If RPM falls back near baseline for some time, switch back to ASSIST
            if rpm < (rpm_baseline + DETECTION_RPM_INCREASE * 0.5):
                # wait a little to prevent chatter
                if (time.time() - gen_engaged_time) > 0.5:
                    print("Generator: load reappeared or RPM fell -> switching back to ASSIST")
                    state = "ASSIST"
                    # refresh baseline
                    rpm_baseline = rpm
